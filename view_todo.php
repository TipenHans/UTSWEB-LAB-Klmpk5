...
<head>
    ...
    <script>
        // JavaScript function to handle countdown for each task
    ...
    deadlineElements.forEach(element => {
        ...
        function updateCountdown() {
            ...
            if (distance > 0) {
                ...
                overdueElement.textContent = ''; // Hide overdue message if not overdue
            } else {
                // Show only the overdue message when the task is overdue
                countdownElement.textContent = ''; // Hide countdown message
                ...
                earlyCompletionElement.textContent = ''; // Clear early completion message if overdue
            }
        }
        ...
        if (element.querySelector('.badge').textContent.trim() === 'Completed') {
            clearInterval(intervalId); // Stop the countdown if the task is completed
            countdownElement.textContent = ''; // Clear the countdown display
            overdueElement.textContent = ''; // Clear the overdue display
            
            // Calculate early completion time
        ...
        // Attach event listener to update the countdown on completion status change
        ...
        updateForm.addEventListener('submit', (event) => {
            ...
            if (completionStatus == 1) {
                clearInterval(intervalId); // Stop countdown if task is marked as completed
                countdownElement.textContent = ''; // Clear the countdown display
                overdueElement.textContent = ''; // Clear the overdue display
                ...
                // If task is marked incomplete, check for overdue
                if (now > deadline) {
...
<div class="container mt-5">
    ...
    <div>
        ...
        <span class="text-success early-completion"></span> <!-- New element for early completion message -->
    </div>
    <div>
        <form method="post" action="update_task_status.php" class="d-inline">
            ...
            <a href="" class="btn btn-sm btn-info">Add Description</a>
